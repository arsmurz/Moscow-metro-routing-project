...
bool BinaryStrategy::save(const FastModificationGraph &graph,
...
void BinaryStrategy::writeStations(std::ofstream &file,
                                   const FastModificationGraph &graph) {
...
void BinaryStrategy::writeEdges(std::ofstream &file,
                                const FastModificationGraph &graph) {
  ...
  graph.forEachVertex([&](const Station &station) {
    ...
    graph.forEachEdge(from, [&](const Edge &e) {
      ...
      file.write(reinterpret_cast<const char *>(&e.is_transfer),
                 sizeof(e.is_transfer));
      return true;
    });
    return true;
  });
...
void BinaryStrategy::readStations(std::ifstream &file,
                                  FastModificationGraph &graph) {
  uint32_t count;
...
void BinaryStrategy::readEdges(std::ifstream &file,
                               FastModificationGraph &graph) {
  uint32_t edge_count;
...
bool JSONStrategy::save(const FastModificationGraph &graph,
                        const std::string &filename) {
  json j;
  j["version"] = 2;
  ...
  graph.forEachVertex([&](const Station &station) {
    stations.push_back({{"id", station.getId()},
...
bool TextStrategy::save(const FastModificationGraph &graph,
                        const std::string &filename) {
  std::ofstream file(filename);
  if (!file) {
    throw FileNotFoundException(filename);
  ...
  graph.forEachVertex([&](const Station &station) {
    file << "STATION " << station.getId() << " " << station.getName() << " "
         << station.getLineNo() << "\n";
    return true;
  });
  ...
  graph.forEachVertex([&](const Station &station) {
    size_t from = station.getId();
    graph.forEachEdge(from, [&](const Edge &e) {
      file << "EDGE " << from << " " << e.to << " " << e.time << " " << e.cost
           << " " << (e.is_transfer ? 1 : 0) << "\n";
      return true;
    });
    return true;
...
bool GraphSerializer::save(const FastModificationGraph &graph,
                           const std::string &filename) {
...
GraphSerializer::createStrategy(const std::string &format) {
  if (format == "binary")
    return std::make_unique<BinaryStrategy>();
  if (format == "json")
    return std::make_unique<JSONStrategy>();
  if (format == "text")
