...
void ConsoleUI::printHelp() const {
    ...
    std::cout << "  add_station <имя> <линия>                - добавить станцию\n";
    std::cout << "  add_edge <откуда> <куда> <время> <цена> [1/0] - добавить ребро (1 - пересадка)\n";
    std::cout << "  list_stations                            - показать все станции\n";
    std::cout << "  list_edges                               - показать все рёбра\n";
...
void ConsoleUI::addEdgeCommand(const std::string& from, const std::string& to, 
    ...
    if (facade.addEdge(from, to, time, cost)) {
        ...
        std::cout << "Ошибка: не удалось добавить ребро (проверьте существование станций)\n";
    }
}
