...
bool NavigatorFacade::addEdge(const std::string& from_name, const std::string& to_name, 
                               int time, int cost) {
    try {
        size_t from_id = graph_->getId(from_name);
        size_t to_id = graph_->getId(to_name);
        graph_->AddEdge(from_id, Edge(time, cost, to_id, (graph_->getLineNum(from_id) == graph_->getLineNum(to_id))));
        return true;
...
std::optional<AllRoutesResult> NavigatorFacade::find_all_routes(const std::string& start_name, 
    ...
    try {
        ...
        auto future_fast = std::async(std::launch::async, [&]() { 
            return run_search(FastestStrategy{}); 
        });
        
        auto future_cheap = std::async(std::launch::async, [&]() { 
            return run_search(CheapestStrategy{}); 
        });
