...
template <> struct equal_to<Edge> {
  bool operator()(const Edge &lhs, const Edge &rhs) const {
    return lhs.to == rhs.to && lhs.time == rhs.time && lhs.cost == rhs.cost &&
...
public:
  ...
  virtual void
  forEachVertex(std::function<bool(const Station &)> visitor) const = 0;
...
public:
  ...
  void
  forEachVertex(std::function<bool(const Station &)> visitor) const override {
  ...
  const Station &getStationById(size_t station_id) const {
    return station_by_id.at(station_id);
  }
  std::string getStationName(size_t station_id) const {
    return station_by_id.at(station_id).getName();
  }
  size_t getStationCount() const { return stations.size(); }
  // virtual size_t getId(std::string name) const override { return
  // name_to_id.at(name); }
  ...
  virtual int32_t getLineNum(size_t Station_id) const override {
    return station_by_id.at(Station_id).getLineNo();
  }
  // std::string getName(size_t Station_id) const { return
  // station_by_id.at(Station_id).getName(); }
