...
public:
    EdgeNotFoundException(size_t from, size_t to)
        : GraphException("Ребро " + std::to_string(from) + " -> " + std::to_string(to) + " не найдено") {}
};
...
public:
    explicit SerializationException(const std::string& msg) : MetroException(msg) {}
};
...
public:
    UnsupportedVersionException(int version, int expected)
        : SerializationException("Неподдерживаемая версия " + std::to_string(version) + 
                                 ". Ожидается " + std::to_string(expected)) {}
};
...
public:
    NoPathFoundException(const std::string& from, const std::string& to)
        : RoutingException("Маршрут от '" + from + "' до '" + to + "' не найден") {}
};