...
public:
    ...
    virtual bool save(const FastModificationGraph& graph, const std::string& filename) = 0;
...
public:
    bool save(const FastModificationGraph& graph, const std::string& filename) override;
...
private:
    ...
    std::unique_ptr<ISerializationStrategy> createStrategy(const std::string& format);
};